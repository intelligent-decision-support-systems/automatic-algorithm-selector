...
public class Main {
	...
	public static void main( String[] args ) {
		...
		options.addOption("id", true, "61");
		options.addOption("config", false, "The config string describing the settings for API interaction");
		//options.addOption("f", true, "The function to invole");
		options.addOption("f", true, "process_dataset");
		...
		try {
			...
			if( cli.hasOption("-config") == false ) {
				config = new Config();
				System.out.println("creating configurations from file.");
				System.out.println(config.getServer() +", "+ config.getUsername()+", "+ config.getPassword());
			} else {
			...
			if( cli.hasOption("f") ) {
				...
				if( function.equals("evaluate_run") ) {
					...
					System.out.println("method: extract_features_all");
